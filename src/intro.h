...
private:
    ...
    void initTitles();
    bool updateTitle();
    bool titleWait(unsigned int msec);
...
private:
    ...
    // Set true by the config menu's "Use These Settings" when the text style
    // changed. The reload of all graphics assets is deferred until the config
    // menu is fully exited (back at the main menu) to avoid repainting while
    // the menu is still open. See keyPressed()'s 'c' handler.
