...
void City::addPeople() {
    ...
    for (current = persons.begin(); current != persons.end(); current++) {
        ...
        // Skip companions who have already joined the party.
        // We identify companions via their NPC type (set from personroles)
        // and match their creature class to the save game player records.
        if (p->getNpcType() == NPC_TALKER_COMPANION) {
            int klass = p->getId() - 2;
            bool joined = false;
            for (int i = 1; i < c->saveGame->members; i++) {
                if (c->saveGame->players[i].klass == klass) {
                    joined = true;
                    break;
                }
            }
