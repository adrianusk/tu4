...
/**
 * Finds direction toward target, preferring the current heading to avoid
 * turning in place (used for pirate ships).
 */
...
Creature *Map::moveObjects(const Coords& avatar) {
    ...
    /* Verify attacker is still in the objects deque (it may have been
       destroyed by another creature's ranged attack during the loop). */
    if (attacker && !objectPresent(attacker))
        attacker = NULL;
    ...
    for (d = DIR_WEST; d <= DIR_SOUTH; d = (Direction)(d+1)) {
        ...
        if (isAvatar) {
            ...
            if (transTile->isShip() && tile->isSailable())
                ...
                if (tile->canWalkOn(d) && prev_tile->canWalkOff(d))
                    retval = DIR_ADD_TO_MASK(d, retval);
            }
