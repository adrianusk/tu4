...
static void frameSleepInit(FrameSleep* fs, int frameDuration) {
    ...
    fs->ftimeIndex = 0;
    fs->fsleep = 0;  /* VSYNC handles frame pacing */
}
...
bool EventHandler::wait_msecs(unsigned int msec) {
    ...
    return wait_msecs(msec, &waitCon);
}

/*
 * Like wait_msecs(unsigned int), but routes input to the given
 * controller instead of a dummy Controller that just consumes key
 * events. Useful for blocking sequences (e.g. intro title animation)
 * that still need to respond to a skip keypress via the controller's
 * own keyPressed() handler.
 *
 * \return true if game should exit.
 */
