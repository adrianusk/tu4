    ...
    for (current = items.begin(); current != items.end(); current++)
    {
        ...
        if (mi->isVisible())
        {
            ...
            if (mi->isHighlighted())
            {
                ...
                view->setCursorPos(mi->getX() - (view->getWidth() == 30 ? 1 : 2), mi->getY(), true);
                view->enableCursor();
            }
            else
            {
...
/**
 * Default redraw: clear the view then show the menu. This substitutes
 * for xu4's pixel-mode background image redraw (BKGD_OPTIONS_TOP/BTM)
 * which we don't perform in text mode, so hidden/resized items don't
 * leave stale characters. Subclasses that draw content outside the
 * menu items (e.g. reagent mixing shortcut letters) should override
 * this to redraw without erasing that external content.
 */
void MenuController::redrawMenu() {
    view->clear();
    menu->show(view);
}
