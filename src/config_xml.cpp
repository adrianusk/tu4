...
static void loadSubImage(ConfigXML* cfg, SubImage* subimage, const ConfigElement &conf) {
    ...
    subimage->left = conf.getInt("left");
    subimage->top  = conf.getInt("top");
    subimage->ystart   = conf.getInt("ystart");
    subimage->yspacing = conf.getInt("yspacing");

    subimage->delay    = conf.getInt("delay", -1);
    subimage->duration = conf.getInt("duration", -1);

    // Map method string to AnimType integer (-1 if not specified)
    subimage->method = -1;
    if (conf.exists("method")) {
        string m = conf.getString("method");
        if (m == "signature")       subimage->method = 0;
        else if (m == "and")        subimage->method = 1;
        else if (m == "bar")        subimage->method = 2;
        else if (m == "origin")     subimage->method = 3;
        else if (m == "present")    subimage->method = 4;
        else if (m == "title")      subimage->method = 5;
        else if (m == "subtitle")   subimage->method = 6;
        else if (m == "map")        subimage->method = 7;
    }
...
static uint8_t mapFiletype(const string& str, ImageInfo* info) {
    if (! str.empty()) {
        ...
        if (str == "image/x-asp-tiles")
            return FTYPE_ASP_TILES;
        if (str == "image/x-asp-screen")
            return FTYPE_ASP_SCREEN;
        if (str == "image/x-asp-charset")
            return FTYPE_ASP_CHARSET;
        if (str == "image/x-asp-moonphases")
            return FTYPE_ASP_MOONPHASES;
        if (str == "image/x-asp-dungeon")
            return FTYPE_ASP_DUNGEON;
    }
    ...
    if (length >= 4 && file.compare(length - 4, 4, ".png") == 0)
        return FTYPE_PNG;
    if (length >= 4 && (file.compare(length - 4, 4, ".ASP") == 0 ||
                        file.compare(length - 4, 4, ".asp") == 0))
        return FTYPE_ASP_SCREEN;
...
static ImageInfo* loadImageInfo(ConfigXML* cfg, const ConfigElement &conf) {
    ...
    /* Optional card-slot placement for the gypsy character-creation scene
     * (used on the abacus image). Default -1 = unspecified. */
    info->cardLeftX  = conf.getInt("leftx",  -1);
    info->cardRightX = conf.getInt("rightx", -1);
    info->cardTop    = conf.getInt("top",    -1);
...
static TileAnimTransform* conf_createAnimTransform(const ConfigElement& conf) {
    static const char* transformTypeStrings[] = {
        "invert", "char_alt", "scroll", "frame", "pixel_color",
        "char_color_alt", "color_flicker", NULL
    };
    ...
    case 1: /* char_alt */
        trans = new TileAnimTransform;
        trans->init(ATYPE_CHAR_ALT);
        trans->var.charAlt.x = conf.getInt("x");
        trans->var.charAlt.y = conf.getInt("y");
        trans->var.charAlt.altchar = conf.getInt("altchar");
        trans->var.charAlt.current = 0;
        break;
    case 2:
        trans = new TileAnimTransform;
    ...
    case 4:
        ...
        break;
    case 5: /* char_color_alt - color alternation at position */
        trans = new TileAnimTransform;
        trans->init(ATYPE_COLOR_ALT);
        trans->var.colorAlt.x = conf.getInt("x");
        trans->var.colorAlt.y = conf.getInt("y");
        trans->var.colorAlt.altcolor = conf.getInt("altcolor");
        trans->var.colorAlt.current = 0;
        break;
    case 6: /* color_flicker */
        trans = new TileAnimTransform;
        trans->init(ATYPE_INVERT);  /* placeholder */
        trans->var.invert.x = conf.getInt("x");
        trans->var.invert.y = conf.getInt("y");
        trans->var.invert.w = conf.getInt("width");
        trans->var.invert.h = conf.getInt("height");
