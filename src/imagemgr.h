...
#define BKGD_MOONPHASES     ImageMgr::sym.moonphases
...
public:
    ...
    /* Card placement for the character-creation (gypsy) scene, read from the
     * abacus image's leftx/rightx/top attributes. Choice A draws at (leftx,top),
     * choice B at (rightx,top). -1 = unspecified (drawCard falls back to the
     * card's own crop location). */
    int16_t cardLeftX, cardRightX, cardTop;
    uint8_t depth;
    uint8_t prescale;
    uint8_t filetype;
