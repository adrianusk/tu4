...
#define SCALED_BASE tu4.settings->scale
#define SCALED_VAR  int sscale = tu4.settings->scale;
...
/* XML DTD validation is a developer-only sanity check. It is disabled by
 * default because libxml2's DTD/XInclude handling produces false
 * "No declaration for element" errors on valid config files (the config
 * loader already treats these as non-fatal -- see config_xml.cpp), and on
 * Windows each warning surfaces as an error dialog. Set validateXml=1 in
 * the settings file to re-enable it when editing the XML/DTD. */
...
public:
    ...
    std::string         textStyle;      // Text-mode graphics theme name
